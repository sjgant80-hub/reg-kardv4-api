// reg-kardv4-api · Express HTTP wrapper around reg-kardv4-sdk · MIT · AI-Native Solutions
import express from 'express';

const app = express();
app.use(express.json({ limit: '10mb' }));

app.get('/health', (_req, res) => res.json({ ok: true, tool: 'reg-kardv4', version: '1.0.0' }));

app.post('/loadRegistry', async (req, res) => {
  try {
    const { loadRegistry } = await import('@ai-native-solutions/reg-kardv4-sdk');
...
app.post('/renderChips', async (req, res) => {
  try {
    const { renderChips } = await import('@ai-native-solutions/reg-kardv4-sdk');
...
app.post('/matchesQuery', async (req, res) => {
  try {
    const { matchesQuery } = await import('@ai-native-solutions/reg-kardv4-sdk');
...
app.post('/card', async (req, res) => {
  try {
    const { card } = await import('@ai-native-solutions/reg-kardv4-sdk');
...
app.post('/escapeHtml', async (req, res) => {
  try {
    const { escapeHtml } = await import('@ai-native-solutions/reg-kardv4-sdk');
...
app.post('/escapeAttr', async (req, res) => {
  try {
    const { escapeAttr } = await import('@ai-native-solutions/reg-kardv4-sdk');
...
app.listen(PORT, () => console.log('reg-kardv4-api listening on :' + PORT));
